...
def debug():
    ...
    print (f'Wind speed is : {wind_speed_m_s} m/s | Limited wind speed is: {get_limited_wind_speed(wind_speed_m_s)} m/s')
    print (f'UV Index is : {uv_intensity} mW/cm^2')
...
if __name__ == "__main__":
    print("Running the first measurement to check the system...\n")
    debug()
    print("First measurement completed...\n")   
    ...
    while True:
        ...
        print(f'\nReading started at: {gps_latitude}, {gps_longitude}, {gps_altitude}{gps_altitude_units}, {gps_datetime}')
        print(f'Temp is : {temp} C, Humidity is: {humidity} %RH')
        ...
        # field names 
        header = ['Date', 'Time', 'Altitude (m)', 'Longitude', 'Latitude', 't (°C)', 'RH (%)', 'Tg(°C)', 'v (m/s)', 'v [0,5-17] (m/s)' 'PM (2.5) ppm', 'PM (10) ppm', 'UV Index'] 
        write_header = False
        try:
